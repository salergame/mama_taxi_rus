...
class _LoyaltyScreenState extends State<LoyaltyScreen> {
  ...
  Future<void> _testAddPoints() async {
    ...
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Тестовые баллы начислены! +5 баллов'),
          backgroundColor: Colors.green,
        ),
      );
      // Перезагружаем данные с небольшой задержкой для синхронизации БД
      await Future.delayed(const Duration(milliseconds: 500));
      _loadLoyaltyData();
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Ошибка начисления баллов'),
            ...
            child: Row(
              children: [
                _buildLoyaltyLevelCard(
                  icon: 'assets/icons/loyalty/coin.svg',
                  points: '0-50',
                  reward: 'Скидка 5%',
                  isActive: _loyaltyData!.points < 50,
                ),
                ...
                _buildLoyaltyLevelCard(
                  ...
                  points: '50-100',
                  reward: 'Скидка 10%',
                  isActive: _loyaltyData!.points >= 50 && _loyaltyData!.points < 100,
                ),
                const SizedBox(width: 12),
                _buildLoyaltyLevelCard(
                  ...
                  points: '100+',
                  reward: 'Скидка 15%',
                  isActive: _loyaltyData!.points >= 100,
                ),
              ],
            ),
          ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 19),
          Column(
            children: _loyaltyData!.history.take(5).map((historyItem) {
  ...
  // Получить иконку для различных типов
  IconData _getIconData(String iconPath) {
    if (iconPath.contains('coin')) return Icons.monetization_on;
    if (iconPath.contains('gift')) return Icons.card_giftcard;
